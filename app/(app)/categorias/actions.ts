"use server"

import { prisma } from "@/lib/prisma"
import { CategoryType } from "@/lib/generated/prisma/client"
import { revalidatePath } from "next/cache"

export async function getCategories() {
  return prisma.category.findMany({
    orderBy: { name: "asc" },
    include: { concepts: { orderBy: { name: "asc" } } },
  })
}

export async function createCategory(name: string, type: CategoryType) {
  await prisma.category.create({ data: { name, type } })
  revalidatePath("/categorias")
}

export async function updateCategory(id: string, name: string, type: CategoryType) {
  await prisma.category.update({ where: { id }, data: { name, type } })
  revalidatePath("/categorias")
}

export async function deleteCategory(id: string) {
  await prisma.category.delete({ where: { id } })
  revalidatePath("/categorias")
}

export async function createConcept(name: string, categoryId: string) {
  await prisma.concept.create({ data: { name, categoryId } })
  revalidatePath("/categorias")
}

export async function updateConcept(id: string, name: string, categoryId?: string, paymentUrl?: string) {
  await prisma.concept.update({
    where: { id },
    data: { name, ...(categoryId ? { categoryId } : {}), paymentUrl: paymentUrl || null },
  })
  revalidatePath("/categorias")
}

export async function toggleConceptRecurring(id: string, recurring: boolean) {
  await prisma.concept.update({ where: { id }, data: { recurring } })
  revalidatePath("/categorias")
  revalidatePath("/")
}

export async function setConceptRecurringMonths(id: string, months: number[]) {
  await prisma.concept.update({ where: { id }, data: { recurringMonths: months } })
  revalidatePath("/categorias")
  revalidatePath("/")
}

export async function toggleConceptActive(id: string, active: boolean) {
  await prisma.concept.update({ where: { id }, data: { active } })
  revalidatePath("/categorias")
  revalidatePath("/transacciones")
}

export async function deleteConcept(id: string) {
  await prisma.concept.delete({ where: { id } })
  revalidatePath("/categorias")
}
